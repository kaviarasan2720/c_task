...
document.title="User log activity | CRM 42andmore";
useEffect(() => {
  loglist();
  alluserlist();
  }, []);
  const clearfilter = () =>{
    ...
    const apiUrl= API +'user/userloglist';
...
/*const DrawerBody = () => {
  ...
  return (
      <div className="drawerpadding formcheckboxdesign">
          <p className="drawertitle">Column By<span className="closebutton" onClick={toggleBottom}>X</span></p>
          <CFormCheck id="sno" label="S.no" checked={checked} onChange={handleChange} />
          <p>Is "My Value" checked? {checked.toString()}</p>
          <CFormCheck id="accesscode" label="Access Code" />
          <CFormCheck id="username" label="Username" />
          <CFormCheck id="useremailid" label="User EmailId" />
          <CFormCheck id="usermobile" label="User Mobile" />
          <CFormCheck id="activity" label="Activity" />
          <CFormCheck id="ipaddress" label="IP Address" />
          <CFormCheck id="date" label="Date" />
          <CFormCheck id="time" label="Time" />
    ...
    return (
        ...
        <CRow>
            ...
            <input name="search" id="product-search-lead" onChange={searchValue} placeholder="Search Username or Access code"/>
            ...
            <CTable align="middle" className="mb-0" border responsive>
                <CTableHead className="leadheadcolor" color="light">
                  <CTableRow>
                    ...
                    {activitychecked ? <CTableHeaderCell>Activity</CTableHeaderCell>:''}
                ...
                {logActivity.map((log , index)=> (
                    ...
                    {codechecked ? <CTableDataCell>42andmore{log.userId}</CTableDataCell>:''}
                    {namechecked ? <CTableDataCell>{log.userName}</CTableDataCell>:''}
                    {emailchecked ? <CTableDataCell>{log.userEmailId}</CTableDataCell>:''}
                    {mobilechecked ? <CTableDataCell>{log.userMobilenumber}</CTableDataCell>:''}
                    {activitychecked ? <CTableDataCell>{log.action}</CTableDataCell>:''}
                    {ipchecked ? <CTableDataCell>{log.ipaddress}</CTableDataCell>:''}
                    {datechecked ? <CTableDataCell>{log.date}</CTableDataCell>:''}
                    {timechecked ? <CTableDataCell>{log.time}</CTableDataCell>:''}
                    </CTableRow>
                  ))}
...
export default User_log_activity;
