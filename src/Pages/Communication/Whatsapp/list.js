...
function Communication_whatsapp() 
{
...
useEffect(() => {
  ...
  const clearfilter = () =>{
    ...
    const apiUrl= API +'whatsapp/whatsapplist';
  ...
  const showChange = (event) =>{
    ...
    const deletefile = (id) =>{
      const apiUrl= API +'whatsapp/deletewhatsapp';
      const myHeaders = new Headers();
      myHeaders.append('Content-Type', 'application/json');
      var raw = JSON.stringify({
          "userId": localStorage.getItem('UserId'),
      ...
      {
        whatsapplist();
        toast.success('Whatsapp Deleted Successfully...', {position: "top-right",autoClose: 1500,hideProgressBar: false,closeOnClick: true,pauseOnHover: true,draggable: true,progress: undefined,theme: "colored",});
    ...
    <CCard className="mb-2 paddingzero">
            ...
            <CCol sm={5}>
              ...
              <div className='leadtrendbutton'>Whatsapp Listing</div>
              </h4>
            </CCol>
        ...
        <CRow>
            ...
            <CIcon icon={cilSearch} size="lg" />
                ...
                <CCol md={6}>
                    ...
                    <Link to="/communication/whatsapp/create"><li><CIcon icon={plus} customClassName="nav-icon"/><div>Template</div></li></Link>
            ...
            <CTable align="middle" className="mb-0" border responsive>
                ...
                {whatsapp.map((whatsapp , index)=> (
                    <CTableRow v-for="item in tableItems">
                      ...
                      {templateNamechecked ? <CTableDataCell>{whatsapp.templateName}</CTableDataCell>:''}
                      {contentchecked ? <CTableDataCell>{whatsapp.content}</CTableDataCell>:''}
                      {datechecked ? <CTableDataCell>{whatsapp.createDate}</CTableDataCell>:''}
                      {statuschecked ?  <CTableDataCell>
                      {(whatsapp.status)==1?<div class="leadstatus activecolorset marginzero">
                    ...
                    <Link to={'/communication/whatsapp/edit/' + whatsapp.id}><div className='svgeditwidth editbox'><CIcon icon={edit} customClassName="nav-icon" /></div></Link>
                    <Link to={'/communication/whatsapp/edit/' + whatsapp.id}><div className='svgviewwidth viewbox'><CIcon icon={view} customClassName="nav-icon" /></div></Link>
                        <div onClick={() => deletewhatsapp(`${whatsapp.id}`)} className='svgviewwidth viewbox cursorpointer'><CIcon icon={deleteicon} customClassName="nav-icon" /></div>
                      </CTableDataCell>:''} 
                    </CTableRow>
                     ))}
...
export default Communication_whatsapp;