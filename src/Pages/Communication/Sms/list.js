...
document.title="SMS list | CRM 42andmore";
...
const contenthandleChange = () => { setcontentChecked(!contentchecked);};
const [templateIdchecked, settemplateIdChecked] = useState(true);
const templateIdhandleChange = () => { settemplateIdChecked(!templateIdchecked);};
const [telemarketerIdchecked, settelemarketerIdChecked] = useState(false);
const telemarketerIdhandleChange = () => { settelemarketerIdChecked(!telemarketerIdchecked);};
...
useEffect(() => {
  ...
  const clearfilter = () =>{
    setStartDate('');setEndDate('');setsmstype('');
    const apiUrl= API +'sms/smslist';
  ...
  const showChange = (event) =>{
    ...
    const deletefile = (id) =>{
      const apiUrl= API +'sms/deletesms';
      const myHeaders = new Headers();
      myHeaders.append('Content-Type', 'application/json');
      var raw = JSON.stringify({
          "userId": localStorage.getItem('UserId'),
      ...
      {
        smslist();
        toast.success('SMS Deleted Successfully...', {position: "top-right",autoClose: 1500,hideProgressBar: false,closeOnClick: true,pauseOnHover: true,draggable: true,progress: undefined,theme: "colored",});
    ...
    <CCard className="mb-2 paddingzero">
            ...
            <CCol sm={5}>
              ...
              <div className='leadtrendbutton'>SMS Listing</div>
              </h4>
            </CCol>
        ...
        <CRow>
            ...
            <CIcon icon={cilSearch} size="lg" />
                ...
                <CCol md={6}>
                    ...
                    <Link to="/communication/sms/senderid-list"><li><CIcon icon={plus} customClassName="nav-icon"/><div>Sender ID</div></li></Link>
                    <Link to="/communication/sms/create"><li><CIcon icon={plus} customClassName="nav-icon"/><div>Template</div></li></Link>
            ...
            <CTable align="middle" className="mb-0" border responsive>
                <CTableHead className="leadheadcolor" color="light">
                  <CTableRow>
                    ...
                    {contentchecked ? <CTableHeaderCell>Content</CTableHeaderCell>:''} 
                    {templateIdchecked ? <CTableHeaderCell>Template ID</CTableHeaderCell>:''} 
                    {telemarketerIdchecked ? <CTableHeaderCell>Telemarketer Id</CTableHeaderCell>:''} 
                ...
                {sms.map((sms , index)=> (
                    ...
                    {templateNamechecked ? <CTableDataCell>{sms.templateName}</CTableDataCell>:''} 
                    {senderidchecked ? <CTableDataCell>{sms.senderName}</CTableDataCell>:''} 
                    {contentchecked ? <CTableDataCell>{sms.content}</CTableDataCell>:''} 
                    {templateIdchecked ? <CTableDataCell>{sms.templateId}</CTableDataCell>:''} 
                    {telemarketerIdchecked ? <CTableDataCell>{sms.telemarketerId}</CTableDataCell>:''} 
                    {datechecked ? <CTableDataCell>{sms.createDate}</CTableDataCell>:''} 
                    {statuschecked ?  <CTableDataCell>
                      {(sms.status)==1?<div class="leadstatus activecolorset marginzero">
                    ...
                    <Link to={'/communication/sms/edit/' + sms.id}><div className='svgeditwidth editbox'><CIcon icon={edit} customClassName="nav-icon" /></div></Link>
                    <Link to={'/communication/sms/view/' + sms.id}><div className='svgviewwidth viewbox'><CIcon icon={view} customClassName="nav-icon" /></div></Link>
                    <div onClick={() => deletesms(`${sms.id}`)} className='svgviewwidth viewbox cursorpointer'><CIcon icon={deleteicon} customClassName="nav-icon" /></div>
                      </CTableDataCell>:''} 
                    </CTableRow>
                    ))}
...
<Drawer open={isBottomOpen} onClose={toggleBottom} direction='right' size={300} enableOverlay={enableOverlay}>
  ...
  <div className="filterpadding formcheckboxdesign">
          ...
          <CFormCheck id="content" label="Content" checked={contentchecked} onChange={contenthandleChange}/>
          <CFormCheck id="templateId" label="Template ID" checked={templateIdchecked} onChange={templateIdhandleChange}/>
          <CFormCheck id="telemarketerId" label="Telemarketer ID" checked={telemarketerIdchecked} onChange={telemarketerIdhandleChange}/>
  ...
  <div className="filterpadding formcheckboxdesign noborderdesign">
          ...
          <CCol md={12}>
            <div className="mb-2">
                ...
                <CFormSelect size="lg" name="smstype" id="smstype" onChange={smstypeChange} aria-label="Large select example" value={smstype}>
...
export default Communication_sms;